...
node_children(rb_ast_t *ast, const NODE *node)
{
    ...
    switch (type) {
      ...
      case NODE_INTEGER:
        return rb_ary_new_from_args(1, rb_compile_integer_literal(RNODE_INTEGER(node)));
      case NODE_FLOAT:
        return rb_ary_new_from_args(1, rb_compile_float_literal(RNODE_FLOAT(node)));
      case NODE_RATIONAL:
        return rb_ary_new_from_args(1, rb_compile_rational_literal(RNODE_RATIONAL(node)));
      case NODE_IMAGINARY:
        return rb_ary_new_from_args(1, rb_compile_imaginary_literal(RNODE_IMAGINARY(node)));
      case NODE_ONCE:
